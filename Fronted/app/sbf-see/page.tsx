...
import { ArrowLeft, Ship, Compass, Navigation, Radio, CloudSun, Shield, Leaf, BookOpen, RefreshCw, Anchor, Map, Flashlight, Volume2, Flag, Lightbulb } from "lucide-react"
import { useModulesWithProgress } from "@/lib/swr-hooks"
import { useEffect, useState, useRef } from "react"

const moduleIcons: Record<string, React.ReactNode> = {
  "see-vorschriften-pflichten": <BookOpen className="h-10 w-10 text-white/30" />,
  "see-ausweichen": <Navigation className="h-10 w-10 text-white/30" />,
  "see-lichter": <Ship className="h-10 w-10 text-white/30" />,
  "see-zeichen-toene": <Volume2 className="h-10 w-10 text-white/30" />,
  "see-verkehrstrennungsgebiete": <Compass className="h-10 w-10 text-white/30" />,
  "see-umweltschutz": <Leaf className="h-10 w-10 text-white/30" />,
  "see-navigation": <Map className="h-10 w-10 text-white/30" />,
  "see-wetterkunde": <CloudSun className="h-10 w-10 text-white/30" />,
  "see-sicherheit": <Shield className="h-10 w-10 text-white/30" />,
  "see-funk": <Radio className="h-10 w-10 text-white/30" />,
  "see-basisfragen": <BookOpen className="h-10 w-10 text-white/30" />,
  "see-knoten": <Anchor className="h-10 w-10 text-white/30" />,
  "see-schallsignale": <Volume2 className="h-10 w-10 text-white/30" />,
  "see-lichter-fahrzeuge": <Lightbulb className="h-10 w-10 text-white/30" />,
  "see-zeichen": <Flag className="h-10 w-10 text-white/30" />,
  "see-feuerarten": <Flashlight className="h-10 w-10 text-white/30" />,
}

// Kategorien für die Module (Reihenfolge: SBF See Fragen oben, dann Basisfragen, dann Praxisprüfung)
const moduleCategories = {
  see: {
    title: "SBF See Fragen", 
    description: "Spezifische Fragen für den Sportbootführerschein See",
    slugs: [
      "see-vorschriften-pflichten",
      "see-ausweichen",
      "see-lichter",
      "see-zeichen-toene",
      "see-verkehrstrennungsgebiete",
      "see-umweltschutz",
      "see-navigation",
      "see-wetterkunde",
      "see-sicherheit",
...
export default function SBFSeePage() {
  const { modules, progress, isLoading, isValidating, refresh } = useModulesWithProgress("see")
  ...
  const handleRefresh = async () => {
    ...
    await refresh()
    // Keep overlay for at least 500ms for better UX
    setTimeout(() => setIsRefreshing(false), 500)
  }

  return (
    <div className="min-h-screen relative" style={{ backgroundColor: "var(--theme-bg)" }}>
      <SidebarNav activeCategory="see" onCategoryChange={() => {}} />
                            ...
                            >
                              ...
                              <div className="flex items-center gap-6 w-full">
                                ...
                                <div className="flex-shrink-0">
                                  {moduleIcons[module.slug] || <Ship className="h-10 w-10 text-white/30" />}
                                </div>
                              </div>
                            </div>
                          </Link>
